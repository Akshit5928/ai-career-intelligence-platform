...
function Opportunities({matches}:{matches:Opportunity[]}) {
  ...
  return <section className="panel page">
    ...
    </div><div className="opps full">{filtered.map(x=><OpportunityCard key={x.internship_id} match={x} detailed/>)}{!filtered.length&&<Empty text={matches.length ? "No opportunities match the current filters." : "Run an agent cycle to discover opportunities."}/>}</div></section> }
