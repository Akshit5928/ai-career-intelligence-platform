...
def run_research_v21() -> dict:
    ...
    in error_message while the persisted run remains in a DB-valid terminal state.
    """
    db = get_supabase()
    sources = (
    ...
    for source in sources:
        ...
        run = db.table("research_runs").insert(
            {"source_id": source.get("id"), "status": "running"}
        ).execute().data[0]
        found = new = 0
        ...
        try:
            fallback = (
                ...
                if source.get("source_type") in {"research", "university", "institute"}
                else "AI/ML Intern"
            )
            ...
            # research_runs only permits running/completed/failed. Preserve
            # recoverable adapter failures in error_message instead of writing
            # an invalid status that causes a second database failure.
            status = "completed"
