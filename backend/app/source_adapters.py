...
def _looks_like_job_card(title: str, text: str) -> bool:
    title_lower = title.lower()
    role_terms = ("ai", "machine learning", "data analyst", "data science", "software engineer", "mlops", "generative ai", "llm")
    has_target_role = any(term in title_lower for term in role_terms)
    return has_target_role and _is_internship(f"{title} {text}")
...
def _html_card_candidates(html: str, page_url: str, fallback_role: str, company: str) -> list[dict]:
    ...
    for block in blocks[:250]:
        ...
        if not _looks_like_job_card(title, text):
            continue
